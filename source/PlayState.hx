...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'headspace':
				dialogue = CoolUtil.coolTextFile(Paths.txt('headspace/headspaceDialogue'));
			case 'reverie':
				dialogue = CoolUtil.coolTextFile(Paths.txt('reverie/reverieDialogue'));
			case 'guilty':
				dialogue = CoolUtil.coolTextFile(Paths.txt('guilty/guiltyDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'whitespace':
			{
					...
					var whitespaceBackground:FlxSprite = new FlxSprite(-800, -400).loadGraphic(Paths.image('whitespaceBackground'));
					whitespaceBackground.antialiasing = true;
					whitespaceBackground.scrollFactor.set(0.1, 0.1);
					whitespaceBackground.active = false;
					add(whitespaceBackground);
					
					var whitespaceDoor:FlxSprite = new FlxSprite(500, 210).loadGraphic(Paths.image('whitespaceDoor'));
					whitespaceDoor.setGraphicSize(Std.int(whitespaceDoor.width * 1.25));
					whitespaceDoor.antialiasing = true;
					whitespaceDoor.scrollFactor.set(0.75, 0.75);
					whitespaceDoor.active = false;
					add(whitespaceDoor);

					var whitespaceFloor:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('whitespaceFloor'));
					whitespaceFloor.setGraphicSize(Std.int(whitespaceFloor.width * 1.1));
					whitespaceFloor.updateHitbox();
					whitespaceFloor.antialiasing = true;
					whitespaceFloor.scrollFactor.set(0.9, 0.9);
					whitespaceFloor.active = false;
					add(whitespaceFloor);

					var whitespaceTissues:FlxSprite = new FlxSprite(1475, 775).loadGraphic(Paths.image('whitespaceTissues'));
					whitespaceTissues.setGraphicSize(Std.int(whitespaceTissues.width * 0.6));
					whitespaceTissues.updateHitbox();
					whitespaceTissues.antialiasing = true;
					whitespaceTissues.scrollFactor.set(1.15, 1.15);
					whitespaceTissues.active = false;
					add(whitespaceTissues);

					var whitespaceSketchbook:FlxSprite = new FlxSprite(1165, 700).loadGraphic(Paths.image('whitespaceSketchbook'));
					whitespaceSketchbook.setGraphicSize(Std.int(whitespaceSketchbook.width * 0.5));
					whitespaceSketchbook.updateHitbox();
					whitespaceSketchbook.antialiasing = true;
					whitespaceSketchbook.scrollFactor.set(0.9, 0.9);
					whitespaceSketchbook.active = false;
					add(whitespaceSketchbook);
					...
					var whitespaceLightbulb:FlxSprite = new FlxSprite(926.5, -467.5).loadGraphic(Paths.image('whitespaceLightbulb'));
					whitespaceLightbulb.setGraphicSize(Std.int(whitespaceLightbulb.width * 0.35));
					whitespaceLightbulb.antialiasing = true;
					whitespaceLightbulb.scrollFactor.set(0.8, 0.8);
		...
		switch (curStage)
		{
			//lol there's nothing for this mod -Coalmunist / Stahl
		}
